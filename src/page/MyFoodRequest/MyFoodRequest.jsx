    ...
    return (
        <div className='container mx-auto '>
            ...
            <div className='grid lg:grid-cols-2 gap-3 relative'>
                {
                    isLoading ? <span className='absolute top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%]'><div className="w-20 h-20 border-4 border-dashed rounded-full opacity-100 border-emerald-600 animate-spin dark:border-violet-400 "></div></span> : requestedData.map(item => <RequestedCard key={item._id} requestedData={requestedData} setrequestedData={setrequestedData} item={item}></RequestedCard>)
                }
            </div>
            {
